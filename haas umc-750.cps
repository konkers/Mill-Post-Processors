/**
  ...
  $Revision: 37959 $
  $Date: 2014-10-16 17:32:01 +0200 (to, 16 okt 2014) $
...
properties = {
  ...
  useG187: true // use G187 to set smoothing on the machine
};
...
function onOpen() {
  ...
  if (true) { // note: setup your machine here
    var bAxis = createAxis({coordinate:1, table:true, axis:[0, 1, 0], range:[-35-0.0001,110+0.0001], preference:1});
...
/*
    ...
    machineConfiguration.setHomePositionY(toPreciseUnit(-8.2138, IN));
*/
...
function onSection() {
  ...
  if ((tool.coolant == COOLANT_THROUGH_TOOL) || (tool.coolant == COOLANT_AIR_THROUGH_TOOL)) {
    if (!isFirstSection() && !insertToolCall && (currentCoolantMode != tool.coolant)) {
      onCommand(COMMAND_STOP_SPINDLE);
      forceSpindleSpeed = true;
    }
    setCoolant(tool.coolant);
  } else if ((currentCoolantMode == COOLANT_THROUGH_TOOL) || (currentCoolantMode == COOLANT_AIR_THROUGH_TOOL)) {
    onCommand(COMMAND_STOP_SPINDLE);
  ...
  if (insertToolCall || retracted ||
    ...
    if (!machineConfiguration.isHeadConfiguration()) {
      ...
      writeBlock(
        gMotionModal.format(0),
        gFormat.format(currentSection.isMultiAxis() ? 234 : 43),
        zOutput.format(initialPosition.z),
        hFormat.format(lengthOffset) // yes - output H with multi axis
      );
    } else {
      writeBlock(
        gAbsIncModal.format(90),
